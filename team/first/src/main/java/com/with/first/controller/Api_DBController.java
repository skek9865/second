...
import com.with.first.service.Board_DBService;
...
@RequestMapping("/api")
@Log4j2
public class Api_DBController {

    private final Board_DBService boardService;

    @GetMapping(value = "/get_list_DB")
    ...
    @PostMapping(value = "/register_DB")
    public HttpStatus register(@RequestBody Board_DBDTO dto){
    ...
    @GetMapping(value = "/get_board_DB/{bno}")
    public ResponseEntity getBoard(@PathVariable("bno") Long bno){

        return ResponseEntity.ok(boardService.get(bno));
    }

    @PutMapping(value = "/modify_DB")
    public HttpStatus modify(@RequestBody Board_DBDTO dto){
    ...
    @DeleteMapping(value = "/delete_DB/{bno}")
