...
import com.with.first.service.Board_VisionService;
...
@RequestMapping("/api")
@Log4j2
public class Api_VisionController {

    private final Board_VisionService boardService;

    @GetMapping(value = "/get_list_vision")
    ...
    @PostMapping(value = "/register_vision")
    public HttpStatus register(@RequestBody Board_VisionDTO dto){
    ...
    @GetMapping(value = "/get_board_vision/{bno}")
    public ResponseEntity getBoard(@PathVariable("bno") Long bno){

        return ResponseEntity.ok(boardService.get(bno));
    }

    @PutMapping(value = "/modify_vision")
    public HttpStatus modify(@RequestBody Board_VisionDTO dto){
    ...
    @DeleteMapping(value = "/delete_vision/{bno}")
