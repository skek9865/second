...
import com.with.first.service.Board_NetworkService;
...
@RequestMapping("/api")
@Log4j2
public class Api_NetworkController {

    private final Board_NetworkService boardService;

    @GetMapping(value = "/get_list_network")
    ...
    @PostMapping(value = "/register_network")
    public HttpStatus register(@RequestBody Board_NetworkDTO dto){
    ...
    @GetMapping(value = "/get_board_network/{bno}")
    public ResponseEntity getBoard(@PathVariable("bno") Long bno){

        return ResponseEntity.ok(boardService.get(bno));
    }

    @PutMapping(value = "/modify_network")
    public HttpStatus modify(@RequestBody Board_NetworkDTO dto){
    ...
    @DeleteMapping(value = "/delete_network/{bno}")
